		...
		callService(service) {
			if(this.state.entity_type === 'input_text' || this.state.entity_type === 'var') {
				...
				if(this.state.service[service]) {
					...
					this._hass.callService(this.state.entity_type, 'python_script.set_state', serviceData).then(function(response) { _this.displayMessage(service, true) }, function(error) { _this.displayMessage(service, false) });
				}
			}
		}
